...
1. 关闭一个文件流：（见 [12.7节](../12/12.7.md)）
```go
// 先打开一个文件 f
defer f.Close()
```

2. 解锁一个被锁定的资源（`mutex`）：（见 [9.3节](../09/09.3.md)）
```go
mu.Lock()
defer mu.Unlock()
```
...
4. 从 panic 恢复：（见 [13.3节](../13/13.3.md)）
```go
defer func() {
...
5. 停止一个计时器：（见 [14.5节](../14/14.5.md)）
```go
tick1 := time.NewTicker(updateInterval)
defer tick1.Stop()
```

6. 释放一个进程 p：（见 [13.6节](../13/13.6.md)）
```go
p, err := os.StartProcess(…, …, …)
defer p.Release()
```

7. 停止 CPU 性能分析并立即写入：（见 [13.10节](../13/13.10.md)）
```go
